...
class Provenance(object):
    ...
    def start(self, workflow=False):
        ...
        self.sw_ddsapi = self.doc.agent(
            ROOCS[f"ddsapi_v{ddsapi_version}"],
            {
                prov.PROV_TYPE: PROV_SOFTWARE_AGENT,
                prov.PROV_LABEL: "DDSAPI",
                DCTERMS_SOURCE: f"https://pypi.org/project/ddsapi",
            },
        )
        # workflow
        if workflow is True:
    ...
    def add_operator(self, operator, parameters, collection, output):
        ...
        if self._workflow:
            self.doc.wasAssociatedWith(op, agent=self.sw_ddsapi, plan=self._workflow)
        else:
            self.doc.start(op, starter=self.sw_ddsapi, trigger=self.sw_rook)
