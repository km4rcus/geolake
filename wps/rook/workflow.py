...
class BaseWorkflow(object):
    def __init__(self, output_dir):
        ...
        self.workflow_ops = WorkflowOperations(output_dir)
...
class Workflow(BaseWorkflow):
    ...
    def _run(self, wfdoc):
        # wfdoc['collection'] = wfdoc["inputs"][next(iter(wfdoc["inputs"]))]
        steps = replace_inputs(wfdoc)
        tree = build_tree(wfdoc)
        last_step = ''
        return self._run_tree(steps, tree, "root", last_step)

    def _run_tree(self, steps, tree, step_id, last_step):
        tree_outputs = {}
        for next_step_id in tree.neighbors(step_id):
            data = tree.get_edge_data(step_id, next_step_id)
            LOGGER.debug(f"data={data}")
            if next_step_id != 'output' and not last_step:
                last_step = next_step_id
            tree_outputs[data["arg_id"]] = self._run_tree(steps, tree, next_step_id, last_step)
        ...
        if step_id in steps:
            if step_id == last_step:
                ...
                islast = False
            outputs = self._run_step(step_id, steps[step_id], tree_outputs, islast)
        elif tree_outputs:
            outputs = list(tree_outputs.values())[0]
        LOGGER.debug(f"outputs={outputs}")
        return outputs

    def _run_step(self, step_id, step, inputs=None, islast=False):
        LOGGER.debug(f"run step={step}, inputs={inputs}")
        if inputs:
            step["in"].update(inputs)
        wf_steps[step_id] = step
        if not islast:
            result = [f'{step_id}_output']
        else:
            collection = wf_steps[next(iter(wf_steps))]['in']['collection']
            args = {'collection': collection, 'steps': wf_steps}
            result = self.workflow_ops.call(args)
        if "subset" == step["run"]:
