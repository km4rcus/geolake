...
class Executor():
    ...
    def create_dask_cluster(self, dask_cluster_opts):
        self._worker_id = self._db.create_worker(status='enabled',
                                                 dask_scheduler_port=dask_cluster_opts['scheduler_port'],
                                                 dask_dashboard_address=dask_cluster_opts['dashboard_address'])
        dask_cluster = LocalCluster(n_workers=dask_cluster_opts['n_workers'], 
    ...
    def info(self, channel, method, properties, body):
        ...
        if (oper == 'info'):
            ...
            if (len(m) == 3): # product info
                dataset_id = m[1]
                product_id = m[2]
                response = json.loads(self._datastore.product_info(dataset_id, product_id))
    ...
    def query(self, channel, method, properties, body):
        ...
        try:
            ...
            print(e)
            self._db.update_request(request_id=request_id, worker_id=self._worker_id, status=RequestStatus.FAILED)

        channel.basic_ack(delivery_tag=method.delivery_tag)

    def subscribe(self, etype):
        print(f'subscribe channel: {etype}_queue')
        self._channel.queue_declare(queue=f'{etype}_queue', durable=True)
        self._channel.basic_qos(prefetch_count=1)
        self._channel.basic_consume(queue=f'{etype}_queue', on_message_callback=getattr(self, etype))
...
if __name__ == "__main__":
    ...
    for etype in executor_types:
        if etype == 'query':
            dask_cluster_opts = {}
            dask_cluster_opts['scheduler_port'] = int(os.getenv('DASK_SCHEDULER_PORT', 8188)) 
